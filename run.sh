...
echo "----------- TRAINING -----------"
python3 src/train.py --cuda=True --pretrained-model=roberta_large_transformers --freeze-bert=False --lstm-dim=-1 --language=polish --seed=1 --lr=5e-6 --epoch=50 --use-crf=False --augment-type=all  --augment-rate=0.1 --alpha-sub=0.4 --alpha-del=0.4 --data-path=data --save-path=out_roberta_large_transformers --batch-size=128